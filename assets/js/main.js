var currentEnv = new RegExp('[\?&]env=([^&#]*)').exec(window.location.href);
currentEnv = currentEnv ?currentEnv[1] :'PRD';
currentEnv = currentEnv.toLowerCase();

var apiBaseUrl = "https://leedonline-api.usgbc.org/v1";

if(currentEnv == 'dev' || currentEnv == 'stg' || currentEnv == 'qas')
	apiBaseUrl = "https://leedonline-api-"+currentEnv+".usgbc.org/v1";
else
...
function print_country(country_id) {
	...
	$.getJSON(apiBaseUrl+'/Common/getCountriesAndStates.json', function (data) {
...
$(document).ready(function () {
	...
	$("#leedprice").submit(function () {
		...
		setDescrpition(givenArea, unitType, type, ratingSystem);
		
		$.getJSON(apiBaseUrl+'/Common/getPriceRelatedInfo.json?countryOrCurrency=' + country, function (infodata) {
			if(infodata.data.currency != undefined){
				...
				callTogetVersionPricesInfo(infodata, curSymbol, currency, givenArea, unitType, type, ratingSystem);
			}
			else {
...
function callTogetVersionPricesInfo(infodata, curSymbol, currency, givenArea, unitType, type, ratingSystem) {
	var data = {
		'versionOrCurrency': currency,
	...
	$.getJSON(apiBaseUrl+'/LEEDPricing/getVersionPricesInfo.json', data, function (pricedata) {
...
function createTotalPriceTable(infodata, pricedata, curSymbol) {
 ...
 function setDescrpition(givenArea, unitType, type, ratingSystem) {
     ...
     var textnode = document.createTextNode(givenArea + " "+((unitType == 'IP') ?'sq ft' :'sq m'));
